...
exports.createOrder = functions.region('asia-northeast2').https.onCall(async (data, context) => {
  const serverTimestamp = admin.firestore.FieldValue.serverTimestamp();
  const { order_number, items, total, subtotal, tax_rate, tracking_number, status } = data;
...
exports.setAdmin = functions.region('asia-northeast2').https.onCall((email, context) => {
  if (context.auth.token.admin !== true) {
    throw new functions.https.HttpsError('permission-denied', 'Request not authorized. User must be a admin to fulfill request.');
  }
...
exports.unsetAdmin = functions.region('asia-northeast2').https.onCall((email, context) => {
  if (context.auth.token.admin !== true) {
    throw new functions.https.HttpsError('permission-denied', 'Request not authorized. User must be a admin to fulfill request.');
  }
...
exports.disableUser = functions.region('asia-northeast2').https.onCall((email, context) => {
  if (context.auth.token.admin !== true) {
    throw new functions.https.HttpsError('permission-denied', 'Request not authorized. User must be a admin to fulfill request.');
  }
...
exports.enableUser = functions.region('asia-northeast2').https.onCall((email, context) => {
  if (context.auth.token.admin !== true) {
    throw new functions.https.HttpsError('permission-denied', 'Request not authorized. User must be a admin to fulfill request.');
  }
